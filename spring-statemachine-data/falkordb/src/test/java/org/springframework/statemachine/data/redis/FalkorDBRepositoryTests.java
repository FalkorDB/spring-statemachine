...
/**
 * FalkorDB repository config tests.
 *
 * @author Janne Valkealahti
 */
@EnabledOnFalkorDB
@Tag("falkordb")
public class FalkorDBRepositoryTests extends AbstractRepositoryTests {
	...
	protected void cleanInternal() {
		...
		kvTemplate.delete(FalkorDBRepositoryAction.class);
		kvTemplate.delete(FalkorDBRepositoryGuard.class);
		kvTemplate.delete(FalkorDBRepositoryState.class);
		kvTemplate.delete(FalkorDBRepositoryTransition.class);
		c.close();
	}
	...
	public void testPopulate1() {
		...
		FalkorDBStateRepository stateRepository = context.getBean(FalkorDBStateRepository.class);
		FalkorDBTransitionRepository transitionRepository = context.getBean(FalkorDBTransitionRepository.class);
		assertThat(stateRepository.count()).isEqualTo(3l);
		assertThat(transitionRepository.count()).isEqualTo(3l);

		List<FalkorDBRepositoryState> states = new ArrayList<>();
		stateRepository.findAll().iterator().forEachRemaining(states::add);
		List<FalkorDBRepositoryTransition> transitions = new ArrayList<>();
		transitionRepository.findAll().iterator().forEachRemaining(transitions::add);
		assertThat(states).hasSize(3);
		assertThat(transitions).hasSize(3);
		FalkorDBRepositoryTransition transition1 = transitions.get(0);
	...
	public void testRepository1() {
		...
		FalkorDBStateRepository statesRepository = context.getBean(FalkorDBStateRepository.class);
		FalkorDBRepositoryState stateS1 = new FalkorDBRepositoryState("S1");
		FalkorDBRepositoryState stateS2 = new FalkorDBRepositoryState("S2");
		...
		FalkorDBTransitionRepository transitionsRepository = context.getBean(FalkorDBTransitionRepository.class);
		FalkorDBRepositoryTransition transition = new FalkorDBRepositoryTransition(stateS1, stateS2, "E1");
		transition.setKind(TransitionKind.EXTERNAL);
		transitionsRepository.save(transition);

		assertThat(statesRepository.count()).isEqualTo(2l);

		FalkorDBRepositoryTransition transition2 = transitionsRepository.findAll().iterator().next();
		...
		List<FalkorDBRepositoryState> findByMachineId = statesRepository.findByMachineId("");
	...
	public static class ConfigWithStrings extends StateMachineConfigurerAdapter<String, String> {
		...
		@Autowired
		private FalkorDBStateMachineRepository falkordbStateMachineRepository;
		...
		public StateMachineRuntimePersister<String, String, String> stateMachineRuntimePersister() {
			return new FalkorDBPersistingStateMachineInterceptor<>(falkordbStateMachineRepository);
		}
	}
