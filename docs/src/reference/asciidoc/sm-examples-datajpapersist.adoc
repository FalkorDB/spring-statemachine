...
database). Optionally, you can also enable Redis, MongoDB or FalkorDB.
...
The following example shows how you can use a very similar configuration
to create a bean for FalkorDB:

====
[source,java,indent=0]
----
include::samples/demo/datapersist/StateMachineConfig.java[tags=snippetF]
----
====
...
other backends, enable either the `mongo` profile, the `redis` profile or the `falkordb` profile.
...
# java -jar spring-statemachine-samples-datapersist-{revnumber}.jar --spring.profiles.active=falkordb
----
=====
====
