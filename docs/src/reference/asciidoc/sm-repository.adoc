...
[[sm-repository-persistence-falkordb]]
==== FalkorDB

The actual repository implementation for a FalkorDB is
`FalkorDBStateMachineRepository`, which is backed by the entity class
`FalkorDBRepositoryStateMachine`.

The following example shows the generic way to persist a machine for FalkorDB:

====
[source,java,indent=0]
----
include::samples/DocsFalkorDBRepositorySampleTests1.java[tags=snippetC]
----
====
